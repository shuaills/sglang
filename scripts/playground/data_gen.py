...
def main():
    ...
    # Above code is from BaldEagle. Below is custom sglang engine implementation.
    ...
    llm = sgl.Engine(
        ...
        return_eagle3_hidden_state="1,23,44",
        tp_size=8,
    ...
    for idx, row in tqdm(enumerate(dataset), total=len(dataset)):
        ...
        if len(batch_rows) < args.batch_size and idx + 1 < len(dataset):
            continue
        # Inference to get hidden_states / target_hidden_states
        ...
        for row, output in zip(batch_rows, outputs):
            ...
            # Each element is a 4*5120 dimensional vector
            # First 5120 dimensions are tgt_hs, remaining 3*5120 dimensions are split into 3 parts as hs
            hidden_dim = 5120
            tgt_hs_list = []
            hs_list = [[], [], []]  # 3 layers
    ...
    llm.shutdown()
    print(f"✅ Done! Data written to {outdir}")
