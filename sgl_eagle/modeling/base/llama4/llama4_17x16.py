...
class Llama4TextModel(Llama4PreTrainedModel):
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        for idx, decoder_layer in enumerate(self.layers[: self.config.num_hidden_layers]):
            if idx == len(self.layers[: self.config.num_hidden_layers]) - 3 or idx == len(self.layers[: self.config.num_hidden_layers]) // 2 or idx == 2:
    ...
    def _update_causal_mask(
        ...
        # [MODIFIED] make dtype to torch.float32
        dtype, device = torch.float32, input_tensor.device
...
EntryClass = [Llama4ForCausalLM, Llama4ForConditionalGeneration, Llama4VisionModel]