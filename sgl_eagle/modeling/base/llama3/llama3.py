...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            # if idx==16:
            #     print(idx)
            # 3t if idx==len(self.layers)-4 or idx==len(self.layers)-16 or idx==2:
            # 2t if idx==len(self.layers)-4 or idx==len(self.layers)-16:
            # 3t1 idx==len(self.layers)-4 or idx==len(self.layers)-16 or idx==0:
            # 2t1 idx==len(self.layers)-2 or idx==len(self.layers)-15:
            # 2t2 idx==len(self.layers)-2 or idx==2:
            # 2t3 idx==len(self.layers)-2 or idx==len(self.layers)-22:
            # 3t2 if idx==len(self.layers)-2 or idx==len(self.layers)-16 or idx==2:
            # 3t3 idx==len(self.layers)-2 or idx==len(self.layers)-16 or idx==1:
            # 3t4 idx==len(self.layers)-2 or idx==len(self.layers)-16 or idx==4:
            # 3t5 idx==len(self.layers)-2 or idx==len(self.layers)-16 or idx==8:
